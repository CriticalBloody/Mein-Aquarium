...
struct PflanzenCardView: View {
    ...
    var mainImage: Image? {
    #if os(macOS)
        if let images = pflanze.images, !images.isEmpty, let nsImage = NSImage(data: images[0]) {
            return Image(nsImage: nsImage)
        }
    #elseif os(iOS)
        if let images = pflanze.images, !images.isEmpty, let uiImage = UIImage(data: images[0]) {
        ...
        ZStack {
            HStack(alignment: .center, spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(pflanze.name.isEmpty ? "Unbekannt" : pflanze.name)
                        .font(.headline.weight(.bold))
                        .foregroundColor(.primary)
                        .lineLimit(1)
                    Text(pflanze.art.isEmpty ? "Art unbekannt" : pflanze.art)
                        .font(.subheadline)
                    ...
                    HStack(spacing: 5) {
                        ...
                        Text(pflanze.aquarium?.name ?? "Kein Aquarium")
                    ...
                    if let birthDate = pflanze.plantedDate as Date? {
                        Text(germanAgeString(since: birthDate))
                            .font(.caption2)
                            .foregroundColor(.secondary)
                            .padding(.top, 3)
                    } else {
                        Text("Einpflanzdatum unbekannt")
                            .font(.caption2)
                            .foregroundColor(.secondary)
                            .padding(.top, 3)
                    }
